...
>
  <form class="flex flex-col space-y-2 w-1/3">
    <input
      ...
      class="border-white border-2 w=1/3 text-black"
      required
    />
    <input
      type="email"
    ...
    <select
      ...
      placeholder="Dag"
      class="border-white border-2 text-black"
    >
    ...
    <button
      type="submit"
      on:click={sendInfo}
      class="border-2 bg-green-500 mx-2 px-2">Send in Bestilling</button
    >
  </form>
  <section class="w-1/3 flex flex-col border-white rounded-xl border-2 space-y-2 align-items-center space-x-2">
    <h1 class="w-full bg-inherit border-b-2">Bestilte timer</h1>
